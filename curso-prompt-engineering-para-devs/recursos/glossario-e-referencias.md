...
Termos do curso, em ordem alfabética, com definição curta e o módulo onde
aparecem.

**Agente** — LLM em loop (pensar→agir→observar) com ferramentas, um objetivo e
um critério de parada, capaz de decidir os próprios passos. (Mód. 09)
...
contexto e saídas de escape. (Mód. 00, 07)

**Casos de borda (edge cases)** — entradas atípicas (vazia, tipo errado, dado
ausente, ambígua, maliciosa) que precisam de regras explícitas. (Mód. 02)
...
RAG; o tamanho e as fronteiras dos chunks afetam muito a qualidade. (Mód. 07)

**Contexto (janela de)** — o total de tokens que o modelo "enxerga" ao
responder (prompt + histórico + dados + resposta). Recurso escasso. (Mód. 07)

**Context engineering / engenharia de contexto** — a disciplina de decidir o
que colocar (e o que cortar) da janela de contexto. (Mód. 07)
...
dado. Recomendado: tags XML nomeadas. (Mód. 01)
...
resultado, para saber se está bom e detectar regressão. (Mód. 06)

**Few-shot / one-shot / zero-shot** — quantos exemplos resolvidos você inclui no
prompt (vários / um / nenhum). (Mód. 03)

**Function calling / tool calling** — dar ao modelo funções que ele pode pedir
para chamar; o modelo decide a chamada, seu código executa. (Mód. 08)
...
perceba; detectada comparando a bateria caso a caso. (Mód. 06)

**Regra positiva vs. negativa** — dizer o que fazer (positiva, guia melhor) vs.
o que não fazer (negativa, guarda-corpo). (Mód. 02)

**Saída de escape** — valor explícito para o caso "não sei / não se aplica"
(`null`, "INVALIDO", "não encontrado"), que evita que o modelo invente. (Mód. 02,
04, 07)

**Token** — a unidade em que o modelo processa texto (pedaços de palavra). Base
do custo, do limite de contexto e da contagem. (Mód. 00)

**Vector store / embedding** — representação numérica de texto (embedding) e o
banco que a armazena, usados para busca por similaridade em RAG. (Mód. 07)
...
- **Guia de structured outputs / JSON mode** do seu provedor — para o Pilar 4 na
  prática.
- **Guia de function/tool calling** do seu provedor — para o Módulo 08.
...
- **RAG (Mód. 07):** estude embeddings, estratégias de chunking, re-ranking e
...
- **Avaliação/evals (Mód. 06):** procure frameworks de eval e o padrão
  "LLM-as-a-judge". O conceito (bateria + métrica + regressão) importa mais que
  a ferramenta específica.
- **Agentes (Mód. 09):** estude padrões de agente (planejar-executar, reflexão,
...
testes** (Mód. 06) no modelo novo, veja o que mudou, e ajuste. Quem tem eval
