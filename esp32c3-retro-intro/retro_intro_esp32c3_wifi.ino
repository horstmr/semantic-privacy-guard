...
// ---- servidor web ----
const char HTML_PAGE[] = "<!DOCTYPE html><html><head><meta charset=UTF-8><meta name=viewport content='width=device-width,initial-scale=1'><title>Pixel Quest</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;background:linear-gradient(135deg,#1a1a2e 0%,#16213e 100%);color:#fff;min-height:100vh;padding:20px;display:flex;align-items:center;justify-content:center}.container{background:rgba(0,0,0,0.6);border-radius:15px;padding:30px;max-width:400px;width:100%;box-shadow:0 8px 32px rgba(31,38,135,0.37);border:2px solid #fcd804}h1{text-align:center;margin-bottom:30px;color:#fcd804;text-shadow:2px 2px 4px rgba(0,0,0,0.5)}.control-group{margin-bottom:25px}label{display:block;margin-bottom:10px;font-weight:bold;color:#fcd804}input{width:100%;padding:10px;border:none;border-radius:8px;font-size:14px;background:rgba(255,255,255,0.1);color:#fff;border:1px solid #fcd804}input[type=range]{padding:0;cursor:pointer}.button-group{display:flex;gap:10px;margin-bottom:20px}button{flex:1;padding:12px;border:none;border-radius:8px;font-weight:bold;cursor:pointer;transition:all 0.3s;font-size:14px;color:white}.play{background:#00a854}.play:hover{background:#00d65c;transform:scale(1.05)}.pause{background:#d9534f}.pause:hover{background:#ff6e6e;transform:scale(1.05)}.color{background:#9966ff}.color:hover{background:#bb99ff;transform:scale(1.05)}.speed-display{text-align:center;color:#fcd804;margin-top:10px;font-weight:bold}.info{background:rgba(31,38,135,0.37);border-left:4px solid #fcd804;padding:15px;border-radius:8px;font-size:13px;line-height:1.6}</style></head><body><div class=container><h1>PIXEL QUEST</h1><div class=control-group><label>Titulo:</label><input type=text id=title value='PIXEL QUEST' maxlength=20></div><div class=control-group><label>Subtitulo:</label><input type=text id=subtitle value='- nivel 1 -' maxlength=15></div><div class=control-group><label>Velocidade:</label><input type=range id=speed min=50 max=150 value=100><div class=speed-display><span id=speedVal>100</span>%</div></div><div class=button-group><button class=play onclick=\"sendCmd('play')\">PLAY</button><button class=pause onclick=\"sendCmd('pause')\">PAUSE</button><button class=color onclick=\"sendCmd('color')\">CORES</button></div><div class=info><strong>Conectado ao:</strong> PIXEL_QUEST<br><strong>IP:</strong> 192 168 4 1<br><strong>Status:</strong> Online</div></div><script>document.getElementById('speed').addEventListener('input',function(e){document.getElementById('speedVal').innerText=e.target.value;fetch('/api/speed?val='+e.target.value)});document.getElementById('title').addEventListener('change',function(e){fetch('/api/title?val='+encodeURIComponent(e.target.value))});document.getElementById('subtitle').addEventListener('change',function(e){fetch('/api/subtitle?val='+encodeURIComponent(e.target.value))});function sendCmd(cmd){fetch('/api/'+cmd)}</script></body></html>";
